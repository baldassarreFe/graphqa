...
native_path = Path('/mnt') / args.native_pdb
decoy_dir = Path('/mnt') / args.decoy_dir
output_file = Path('/mnt') / args.output_file
...
np.savez(output_file.resolve(), **{
    'decoys': [p.with_suffix('').name for p in decoy_paths],
